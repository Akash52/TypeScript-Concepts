...
var subDisplay = document.getElementById('sub_res.display');
var buttons = document.getElementsByClassName('btn');
var btnMC = document.querySelector('#mc');
var btnMR = document.querySelector('#mr');
res.display = document.querySelector('#screen');
...
function syntaxError() {
    if (eval(res.display.value) == SyntaxError ||
...
function equals() {
    if (res.display.value.indexOf('^') > -1) {
        var base = res.display.value.slice(0, res.display.value.indexOf('^'));
        var exponent_1 = res.display.value.slice(res.display.value.indexOf('^') + 1);
        res.display.value = eval('Math.pow(' + base + ',' + exponent_1 + ')');
    }
    else {
